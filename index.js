...
function readHighValueEffectSettings() {
  if (!fs.existsSync(HIGH_VALUE_EFFECT_SETTINGS_PATH)) {
   return { threshold: 100000, animationPath: "", displayMs: 4200 };
  }
...
const HIGH_VALUE_EFFECT_PATH = resolveDataPath("high_value_effect.json");

function readHighValueEffectSettings() {
  if (!fs.existsSync(HIGH_VALUE_EFFECT_PATH)) {
    return {
      threshold: 100000,
      animationPath: "/uploads/promos/promo_1773918193087.mp4",
    };
  }

  try {
    const raw = JSON.parse(fs.readFileSync(HIGH_VALUE_EFFECT_PATH, "utf8"));
    const threshold = Number(raw?.threshold) || 100000;
    const animationPath = String(raw?.animationPath ?? "").trim();
    return {
      threshold: threshold < 1 ? 1 : threshold,
    ...
    console.error("HIGH VALUE EFFECT LOAD ERROR:", e);
    return {
      threshold: 100000,
      animationPath: "/uploads/promos/promo_1773918193087.mp4",
    };
...
app.get("/api/high-value-effect", (req, res) => {
  res.json(readHighValueEffectSettings());
});

app.post("/api/high-value-effect", (req, res) => {
