...
const HIGH_VALUE_EFFECT_SETTINGS_PATH = resolveDataPath("high_value_order_effect.json");

function readHighValueEffectSettings() {
  if (!fs.existsSync(HIGH_VALUE_EFFECT_SETTINGS_PATH)) {
   return { threshold: 100000, animationPath: "", displayMs: 4200 , scale: 1.0};
  }

   try {
   const raw = JSON.parse(fs.readFileSync(HIGH_VALUE_EFFECT_SETTINGS_PATH, "utf8"));
    const threshold = Number(raw?.threshold) || 100000;
    const animationPath = String(raw?.animationPath ?? "").trim();
    const displayMs = Number(raw?.displayMs) || 4200;
    const scale = Number(raw?.scale) || 1.0;
    return {
      threshold: threshold < 1 ? 1 : threshold,
      animationPath,
      displayMs: Math.min(15000, Math.max(1000, displayMs)),
      scale: Math.min(1.0, Math.max(0.4, scale)),
    };
  } catch (e) {
    console.error("HIGH VALUE EFFECT SETTINGS LOAD ERROR:", e);
    return { threshold: 100000, animationPath: "", displayMs: 4200, scale: 1.0 };
  }
}

function saveHighValueEffectSettings(settings) {
  const displayMs = Number(settings?.displayMs) || 4200;
  const scale = Number(settings?.scale) || 1.0;
  const normalized = {
    threshold: Math.max(1, Number(settings?.threshold) || 100000),
    animationPath: String(settings?.animationPath ?? "").trim(),
    displayMs: Math.min(15000, Math.max(1000, Math.floor(displayMs))),
    scale: Math.min(1.0, Math.max(0.4, scale)),
  };

  fs.writeFileSync(
    HIGH_VALUE_EFFECT_SETTINGS_PATH,
...
app.get("/api/tables", (req, res) => {
  try {
    const tables = store.listTableCatalog();
    res.json({ ok: true, tables });
  } catch (e) {
    console.error("TABLE CATALOG LIST ERROR:", e);
...
app.post("/api/tables", (req, res) => {
  try {
    ...
    store.ensureTableCatalog(name);
    broadcastSnapshot();
    res.json({ ok: true, tables: store.listTableCatalog() });
  } catch (e) {
    console.error("TABLE CATALOG ADD ERROR:", e);
    res.status(400).json({ ok: false, error: e.message });
  }
});

app.patch("/api/tables/:oldName", (req, res) => {
  try {
    const oldName = String(req.params.oldName ?? "").trim();
    const newName = String(req.body?.newName ?? "").trim();
    if (!oldName || !newName) {
      return res.status(400).json({ ok: false, error: "old/new required" });
    }
    store.renameTableCatalog(oldName, newName);
    broadcastSnapshot();
    res.json({ ok: true, tables: store.listTableCatalog() });
  } catch (e) {
    console.error("TABLE CATALOG RENAME ERROR:", e);
    res.status(400).json({ ok: false, error: e.message });
  }
});
...
app.delete("/api/tables/:tableId", (req, res) => {
  try {
    ...
    if (!tableId) {
      return res.status(400).json({ ok: false, error: "tableId required" });
    }
    const snap = store.tableOrders?.get(String(tableId));
    if (Array.isArray(snap?.items) && snap.items.length > 0) {
      return res.status(400).json({ ok: false, error: "table has orders" });
    }
    store.removeTableCatalog(tableId);
    broadcastSnapshot();
    res.json({ ok: true, tables: store.listTableCatalog() });
  } catch (e) {
    console.error("TABLE CATALOG DELETE ERROR:", e);
    res.status(400).json({ ok: false, error: e.message });
  }
});
...
app.get("/api/high-value-order-effect", (req, res) => {
  res.json(readHighValueEffectSettings());
});

app.post("/api/high-value-order-effect", (req, res) => {
  const normalized = saveHighValueEffectSettings(req.body || {});
...
app.post("/api/printer-settings", (req, res) => {
  ...
  const normalized = {
    ...
    register: {
      ...
      port: Number(input?.register?.porthigh_value_order_effect.json
      ) || 9100,
    },
    receipt: {
...
function handleGetHighValueOrderEffect(req, res) {
  res.json(readHighValueEffectSettings());
}

function handleSaveHighValueOrderEffect(req, res) {
  const threshold = Number(req.body?.threshold);
  const animationPath = String(req.body?.animationPath ?? "").trim();
  const displayMs = Number(req.body?.displayMs ?? 4200);
  ...
  if (!Number.isFinite(threshold) || threshold < 1) {
   ...
   if (!animationPath) {
    return res.status(400).json({ error: "animationPath required" });
  }
  if (!Number.isFinite(displayMs) || displayMs < 1000 || displayMs > 15000) {
    return res.status(400).json({ error: "displayMs must be between 1000 and 15000" });
  }
  if (!Number.isFinite(scale) || scale < 0.4 || scale > 1.0) {
    return res.status(400).json({ error: "scale must be between 0.4 and 1.0" });
  }
 ...
 const normalized = {
    ...
    animationPath,
     displayMs: Math.floor(displayMs),
    scale,
  };

  saveHighValueEffectSettings(normalized);
  res.json({ ok: true, settings: normalized });
}

// 新API（Flutter側）
app.get("/api/high-value-order-effect", handleGetHighValueOrderEffect);
app.post("/api/high-value-order-effect", handleSaveHighValueOrderEffect);
// 旧API（互換）
app.get("/api/high-value-effect", handleGetHighValueOrderEffect);
app.post("/api/high-value-effect", handleSaveHighValueOrderEffect);
console.log("★★★ここ動いてる★★★");
