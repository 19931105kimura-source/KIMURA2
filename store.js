...
class Store {
  ...
  _bootTableCatalog() {
    ...
    const insert = db.prepare(`
      INSERT INTO table_catalog (id, sort_order, created_at, updated_at)
      VALUES (?, ?, ?, ?)
    `);

    const tx = db.transaction(() => {
      DEFAULT_TABLE_IDS.forEach((id, idx) => {
        const ts = nowIso();
        insert.run(String(id), idx, ts, ts);
      });
    });
  ...
  ensureTableCatalog(tableId) {
    ...
    const row = db.prepare(`
      ...
      FROM table_catalog
    `).get();
    const nextSort = Number(row?.max_sort ?? -1) + 1;
    const ts = nowIso();

    db.prepare(`
      INSERT INTO table_catalog (id, sort_order, created_at, updated_at)
      VALUES (?, ?, ?, ?)
    `).run(normalized, nextSort, ts, ts);

    return normalized;
  }

  renameTableCatalog(oldId, newId) {
    const from = String(oldId ?? "").trim();
    const to = String(newId ?? "").trim();
    if (!from || !to) throw new Error("old/new table id required");
    if (from === to) return;

    const existsFrom = db.prepare(`
      SELECT 1
      FROM table_catalog
      WHERE id = ?
      LIMIT 1
    ...
    `).get(to);
    if (existsTo) {
      throw new Error(`Table already exists: ${to}`);
    }
    ...
    const tableRuntime = this.tables.get(from);
    if (tableRuntime) {
      this.tables.delete(from);
      tableRuntime.tableId = to;
      this.tables.set(to, tableRuntime);
    }

    const snap = this.tableOrders.get(from);
    if (snap) {
      this.tableOrders.delete(from);
      snap.tableId = to;
      this.tableOrders.set(to, snap);
    }

    const orderIds = this.ordersByTable.get(from);
    if (orderIds) {
      this.ordersByTable.delete(from);
      this.ordersByTable.set(to, orderIds);
    }

    for (const order of this.orders.values()) {
      if (order.tableId === from) order.tableId = to;
    }
    for (const item of this.orderItems.values()) {
      if (item.tableId === from) item.tableId = to;
    }
  }

  removeTableCatalog(tableId) {
    const normalized = String(tableId ?? "").trim();
    if (!normalized) throw new Error("tableId is required");
    ...
    this.tableOrders.delete(normalized);
    this.ordersByTable.delete(normalized);
  }
  ...
  // ---------- Table ----------
  openTable(tableId) {
    const normalizedTableId = this.ensureTableCatalog(tableId);
    ...
    if (!this.ordersByTable.has(normalizedTableId)) {
      this.ordersByTable.set(normalizedTableId, []);
    }
    return table;
  }
